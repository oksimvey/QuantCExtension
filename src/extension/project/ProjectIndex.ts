...
export interface IndexedClass { name: string; baseClass?: string; members: IndexedMember[]; start: number; end: number; uri: string }
...
export class ProjectIndex {
  ...
  remove(uri: string): void {
    ...
    for (const [name, value] of this.classes) if (value.uri === uri) this.classes.delete(name);
  }
    ...
    for (const declaration of program.declarations) {
      ...
      const members: IndexedMember[] = c.members.map(member => {
        ...
        if (member.kind === "FunctionDeclaration") {
          ...
          return {
            name: fn.name, typeName: fn.returnType.name, kind: "method" as const, isGlobal,
            parameters: fn.parameters.map(x => ({ name: x.name, typeName: x.type.name })),
            start: fn.start, end: fn.end, uri
          };
        }
        const field = member as VariableDeclarationNode;
        return {
          name: field.name, typeName: field.type.name, kind: "field" as const, isGlobal,
          parameters: [], start: field.start, end: field.end, uri
        };
      });
      this.classes.set(c.name, { name: c.name, baseClass: c.baseClass, members, start: c.start, end: c.end, uri });
    }
  }
}
